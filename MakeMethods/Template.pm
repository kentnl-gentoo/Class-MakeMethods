...
such as the C<hash> accessors provided by the various Basic
...
=head2 Basic Declaration Syntax
...
=head2 Template Declaration Syntax
...
interface, to use for this meta-method.

=item *

modifier

The names of one or more predefined modifier flags.
...
Methods for deep copies and comparisons
...
The most commonly used implementation, for objects based on blessed hashes.
...
For objects based on blessed scalars.
...
name or from any instance will consistently access the same value.
...
value accessible via the symbol table.
...
Class methods are similar to Static methods, except that each subclass and its instances will access a distinct value.
...
ClassInherit methods are an intermediate point between Static and Class methods; subclasses inherit their superclass's value until they set their own value, after which they become distinct.
...
To enable disk caching of generated code, create an empty directory and pass it to the DiskCache package:0

  use Class::MakeMethods::Template::DiskCache qw( /my/code/dir );

This has a mixed effect on performance.
...
then sets the 'join' parameter for that method.
